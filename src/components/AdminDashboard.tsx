      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mb-8 flex flex-wrap gap-4 border-b border-border">
          <button
            ...
            className={`flex items-center gap-2 border-b-2 px-4 py-3 text-sm font-bold transition-colors ${activeTab === 'projects' ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground'}`}
          >
            <Briefcase size={18} /> Projects
          </button>
          <button
            onClick={() => setActiveTab('certificates')}
            className={`flex items-center gap-2 border-b-2 px-4 py-3 text-sm font-bold transition-colors ${activeTab === 'certificates' ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground'}`}
          >
            <Award size={18} /> Certificates
          </button>
          <button
            onClick={() => setActiveTab('announcements')}
            className={`flex items-center gap-2 border-b-2 px-4 py-3 text-sm font-bold transition-colors ${activeTab === 'announcements' ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground'}`}
          >
            <Bell size={18} /> Announcements
          </button>
          <button
            onClick={() => setActiveTab('gallery')}
            className={`flex items-center gap-2 border-b-2 px-4 py-3 text-sm font-bold transition-colors ${activeTab === 'gallery' ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground'}`}
          >
            <ImageIcon size={18} /> Gallery
          </button>
          <button
            onClick={() => setActiveTab('playlist')}
            className={`flex items-center gap-2 border-b-2 px-4 py-3 text-sm font-bold transition-colors ${activeTab === 'playlist' ? 'border-primary text-primary' : 'border-transparent text-muted-foreground hover:text-foreground'}`}
          >
        ...
        <div className="space-y-8">
            ...
            <div className="rounded-[2rem] border border-border bg-card p-8 shadow-xl">
              ...
              <div className="space-y-4">
                {projects.map(project => (
                  <div key={project.id} className="flex flex-col gap-4 rounded-2xl border border-border/50 bg-muted/20 p-6 md:flex-row md:items-center md:justify-between">
                    {isEditingProject === project.id ? (
                      <div className="flex w-full flex-col gap-4">
                        ...
                        <div className="flex gap-2">
                          <button onClick={saveEditProject} className="flex items-center gap-1 rounded-lg bg-green-500 px-4 py-2 text-sm font-bold text-white"><Save size={16} /> Simpan</button>
                          <button onClick={() => setIsEditingProject(null)} className="flex items-center gap-1 rounded-lg bg-muted px-4 py-2 text-sm font-bold"><X size={16} /> Batal</button>
                        </div>
                      </div>
                    ) : (
                      <>
            ...
            <div className="rounded-[2rem] border border-border bg-card p-8 shadow-xl">
              ...
              <div className="space-y-4">
                {certificates.map(cert => (
                  <div key={cert.id} className="flex flex-col gap-4 rounded-2xl border border-border/50 bg-muted/20 p-6 md:flex-row md:items-center md:justify-between">
                    {isEditingCert === cert.id ? (
                      <div className="flex w-full flex-col gap-4">
                        ...
                        />
                        <div className="flex gap-2">
                          <button onClick={saveEditCert} className="flex items-center gap-1 rounded-lg bg-green-500 px-4 py-2 text-sm font-bold text-white"><Save size={16} /> Simpan</button>
                          <button onClick={() => setIsEditingCert(null)} className="flex items-center gap-1 rounded-lg bg-muted px-4 py-2 text-sm font-bold"><X size={16} /> Batal</button>
                        </div>
                      </div>
                    ) : (
                      <>
                        <div className="flex-1">
                          ...
                          <span className="mt-1 inline-block rounded bg-secondary/10 px-2 py-1 text-[10px] font-bold text-secondary">{cert.tag}</span>
                        </div>
                        <div className="flex items-center gap-2">
            ...
            <div className="rounded-[2rem] border border-border bg-card p-8 shadow-xl">
              ...
              <div className="space-y-4">
                {announcements.map(ann => (
                  <div key={ann.id} className="flex flex-col gap-4 rounded-2xl border border-border/50 bg-muted/20 p-6 md:flex-row md:items-center md:justify-between">
                    {isEditingAnn === ann.id ? (
                      <div className="flex w-full flex-col gap-4">
                        <input
                          type="text"
                        ...
                        />
                        <div>
                          <label className="mb-1 block text-xs text-muted-foreground">Hapus Otomatis Pada:</label>
                          <input
                            type="datetime-local"
                        ...
                        <div className="flex gap-2">
                          <button onClick={saveEditAnn} className="flex items-center gap-1 rounded-lg bg-green-500 px-4 py-2 text-sm font-bold text-white"><Save size={16} /> Simpan</button>
                          <button onClick={() => setIsEditingAnn(null)} className="flex items-center gap-1 rounded-lg bg-muted px-4 py-2 text-sm font-bold"><X size={16} /> Batal</button>
                        </div>
                      </div>
                    ) : (
                      <>
                        <div className="flex-1">
                          ...
                          <span className="mt-2 inline-block rounded bg-red-500/10 px-2 py-1 text-[10px] font-bold text-red-500">
          ...
          {activeTab === 'gallery' && (
            <div className="rounded-[2rem] border border-border bg-card p-8 shadow-xl">
              ...
              <div className="space-y-4">
                {gallery.map(item => (
                  <div key={item.id} className="flex flex-col gap-4 rounded-2xl border border-border/50 bg-muted/20 p-6 md:flex-row md:items-center md:justify-between">
                    {isEditingGallery === item.id ? (
                      <div className="flex w-full flex-col gap-4">
                        ...
                        />
                        <div className="flex gap-2">
                          <button onClick={saveEditGallery} className="flex items-center gap-1 rounded-lg bg-green-500 px-4 py-2 text-sm font-bold text-white"><Save size={16} /> Simpan</button>
                          <button onClick={() => setIsEditingGallery(null)} className="flex items-center gap-1 rounded-lg bg-muted px-4 py-2 text-sm font-bold"><X size={16} /> Batal</button>
                        </div>
                      </div>
                    ) : (
                      <>
                        <div className="flex flex-1 items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground truncate max-w-[200px]">{item.imageUrl}</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
          ...
          {activeTab === 'playlist' && (
            <div className="rounded-[2rem] border border-border bg-card p-8 shadow-xl">
              ...
              <div className="space-y-4">
                {playlist.map(song => (
                  <div key={song.id} className="flex flex-col gap-4 rounded-2xl border border-border/50 bg-muted/20 p-6 md:flex-row md:items-center md:justify-between">
                    {isEditingSong === song.id ? (
                      <div className="flex w-full flex-col gap-4">
                        ...
                        />
                        <div className="flex gap-2">
                          <button onClick={saveEditSong} className="flex items-center gap-1 rounded-lg bg-green-500 px-4 py-2 text-sm font-bold text-white"><Save size={16} /> Simpan</button>
                          <button onClick={() => setIsEditingSong(null)} className="flex items-center gap-1 rounded-lg bg-muted px-4 py-2 text-sm font-bold"><X size={16} /> Batal</button>
                        </div>
                      </div>
                    ) : (
                      <>
                        <div className="flex flex-1 items-center gap-4">
                          ...
                          <div>
                            ...
                            <p className="text-xs text-muted-foreground truncate max-w-[200px]">{song.audioUrl}</p>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
