...
export default function Skills() {
  ...
  // If we have skills in Firestore, use them. Otherwise fallback to static data.
  const hasFirestoreSkills = firestoreSkills.length > 0;
          ...
          {hasFirestoreSkills ? (
            firestoreSkills.map((skill, index) => (
              ...
              >
                <div className="mb-6 flex h-14 w-14 items-center justify-center rounded-xl bg-primary/5 text-primary transition-all duration-500 group-hover:scale-110 group-hover:bg-primary group-hover:text-white">
                  {skill.type === 'programming' ? <Code2 size={28} /> : <Cpu size={28} />}
                </div>
                <h3 className="mb-4 text-2xl font-display font-black tracking-tight">{skill.category}</h3>
                <div className="flex flex-wrap gap-2">
                  {skill.items?.map((item: string) => (
                    <span key={item} className="rounded-lg border border-border bg-muted/50 px-4 py-2 text-xs font-bold tracking-tight transition-colors hover:bg-primary hover:text-white">
                      {item}
                    </span>
                  ))}
                </div>
            ...
            <>
              ...
              {CV_DATA.skills.others.map((skill, i) => {
                ...
                const colorClass = isCyber ? 'secondary' : isAI ? 'primary' : 'secondary';
